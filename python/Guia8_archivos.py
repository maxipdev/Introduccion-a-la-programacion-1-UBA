...
def agrupar_por_longitud(nombre_archivo) -> dict: 
    archivo = open(f"archivos-python/{nombre_archivo}", "r")
...
def es_comentario(linea:str) ->bool: 
    for caracter in linea:
        if caracter == "#":
            return True
        elif caracter == " " or caracter == "\t": # se fija si hay un espacio en blanco o si hay un tab, ya que la computadora los toma como 2 diferentes
            continue # que siga fijandose
        else: 
            return False #pq es un caracter valido, como una letra?
    return False ## -> pq esto se llega si son todas lineas en blanco, se ponq es q es valdio, ya que no es un comentario
...
def clonar_sin_comentarios(nombre_archivo_1, nombre_archivo_2):
    ...
    for i in lineas: 
        if not es_comentario(i): 
            archivo2.write(i)
    archivo2.close()

#clonar_sin_comentarios("texto-8.txt", "texto-2.txt")
...
def agregar_frase_al_final(nombre_archivo: str, frase: str): 
    archivo = open(f"archivos-python/{nombre_archivo}", "a")